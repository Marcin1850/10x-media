...
Every error the endpoint returns carries a machine-readable `code` beside the English `error` string, and the interface renders the Polish copy for that code (`src/lib/copy/pl.ts`). The code exists because one status can mean several things — a 422 covers "this video has no caption track", "a transcript came back with no words in it" and "we could not reach the transcript service", which a user must act on differently — so translating one string per status would have merged them. The English `error` stays on the body for logs and non-browser consumers, and an unknown or missing code falls back to a per-status Polish message rather than to English. `src/lib/copy/error-codes.test.ts` fails the build if the endpoint sends a code the copy table has no entry for.
