...
- [x] 5.1 `npx supabase migration list --linked` shows local and remote in sync — 1a33e8a
- [x] 5.2 `wrangler deploy` reports a new version and a live generation returns without a 500 — 1a33e8a

#### Manual

- [x] 5.3 Both live runs behave as specified; total `usedCredits` delta matches `sum(billable_credits)` plus one credit per `unavailable` row, any remaining gap attributed to specific null rows — 1a33e8a
- [x] 5.4 Live telemetry values are plausible (`llm_ms` dominates, `cost_usd` ~1–2 ¢) — 1a33e8a
...
- [x] 5.6 `resolved_via`-as-Whisper-proxy question recorded as open and unbudgeted — answered by observing real traffic, not a paid run — and handed to S-09 — 1a33e8a
- [x] 5.7 Roadmap (status, At a glance, Backlog Handoff) and Linear both updated — 1a33e8a
