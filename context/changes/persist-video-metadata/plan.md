...
>
> Two facts came back. First, `lang` **selects** among existing caption tracks and never requests a translation — Supadata translates only through a separate, explicitly-called endpoint. So dropping `lang: "pl"` never protected anything. Second, and decisively: re-fetching `jNQXAC9IVRw` with `lang: "en"` returned the genuine English original, and re-running `iG9CE55wbtY` did the same where the no-`lang` build had taken `af` — the first entry in its 61-track pool. The vendor's default is literally "first track in the pool", and pool order carries no information about which track is the source. *Omitting `lang` was the exposure, not the protection* — the plan's premise was inverted.
>
> **Confirmed end-to-end 2026-07-27** by re-running rows 2.3 and 2.5: both videos now record `transcript_lang = 'en'`, and the German wording is gone from the delivered summary ("trąby", from the English "trunks", where the previous run said "Rüssel").
>
...
- [x] 2.3 A non-Polish video populates `transcript_lang` with its own language — ~~2026-07-26, `de` on `jNQXAC9IVRw`, `en` on `dQw4w9WgXcQ`, `af` on `iG9CE55wbtY`~~ superseded by the F9 fix; **RE-RUN 2026-07-27**: `jNQXAC9IVRw` `de` → **`en`** (pool `{en,de}`), `iG9CE55wbtY` `af` → **`en`** (pool of 61 beginning `af`). `transcript_lang` records the track actually selected, and `lang: "en"` overrides the vendor's first-in-pool default on both videos that previously got it wrong. Never `pl`
- [x] 2.4 `transcript_available_langs` is a readable Postgres array — 2026-07-26, `array_length` 2 / 5 / 61, `[1]` reads back
- [x] 2.5 The summary is still returned in Polish — ~~2026-07-26, Polish output from `de` and `af` transcripts~~; **RE-RUN 2026-07-27**: both generations returned Polish from English transcripts (200 in 38s and 46s). The fidelity gain is visible in the output — the "Me at the zoo" summary now says "**trąby**" (from the English "trunks") where the 2026-07-26 run said "Rüssel", straight out of the German captions. Credits 9 → 7, both reservations `settled`
