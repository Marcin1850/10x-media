...
After a successful generation the video's row carries all seven fields, with nulls only where Supadata genuinely had nothing or the metadata call failed. The transcript request no longer asks for a Polish caption track, so the model works from whatever track Supadata considers first-available rather than a potentially machine-translated Polish one. Nothing renders yet — S-02 owns that.
...
**In scope:** five new nullable columns on `videos`; a widened `persist_summary`; a new `metadata.ts` service; dropping `lang` from the transcript request and capturing `availableLangs`; wiring both into the generation endpoint; production rollout.

**Out of scope:** any UI; backfilling existing rows; storing thumbnail images; a real "video language" field via the YouTube Data API; the Supadata cost guardrail (S-09); widening the transcript-quote cache.
...
| --- | --- | --- |
| 1. Schema + widened persist path | Columns exist; RPC accepts metadata; nulls passed | The RPC swap is the one irreversible-in-order step — old Worker + new schema is a broken combination |
