...
Supadata API reference captured while scoping S-08 (video metadata) and S-09 (transcript cost guardrail). Fetched via **Context7** (`/llmstxt/supadata_ai_llms_txt`) on **2026-07-25**, plus one live `GET /v1/me` call against the project's own key. **Re-fetched and extended 2026-07-27** during impl-review triage of finding F9, with three live probe calls (one `/v1/transcript`, two `/v1/metadata`) — see `supadata-transcript.md` §Language selection. See `context/foundation/roadmap.md` §S-08 / §S-09 for the decisions these files back; the files themselves are the raw API reference to code against.
...
- ~~**One question is genuinely unanswered by these docs:**~~ **Answered 2026-07-27** — see `supadata-transcript.md` §Language selection. Translated tracks *do* enter the pool, and the vendor's no-`lang` default picked one over the original on both probed videos (an English video with pool `["en","de"]` returned `de` despite `en` being listed first). The docs were correct that `lang` selects rather than translates; what they got wrong by omission is that *omitting* `lang` gives no protection. S-08 now requests `lang: "en"`. Cost: 3 credits, against the "~2 credits" this file estimated.
