...
- **`getViteConfig()` loads the full Astro config** (Cloudflare adapter, two Google font families). If that needs network or dominates run time, the plain `vitest/config` fallback applies and gets recorded in §6.6.
