...
| Server mode | Built `preview` everywhere, never reused | ~~`dev` locally, built `preview` in CI.~~ **Corrected by the Phases 1-2 impl review (F1/F2, 2026-09-09)**: `reuseExistingServer` is `false` in both environments, so the runner fails closed instead of adopting a developer's `npm run dev` — which carries neither the fake-LLM alias nor `.dev.vars.e2e`, and would call OpenRouter for real. ~30 s of build per run is the price of a server that cannot be holding real vendor keys. | Plan, corrected in Phases 1-2 review |
