...
- [ ] 3.3 `summarize()` returns Polish text + non-empty served `model` slug
- [ ] 3.4 Transcript service returns `{ ok: false, reason: "unavailable" }` (no throw) for a caption-less video
