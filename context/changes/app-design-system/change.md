...
**3. Design system project — CREATED 2026-08-11.** `10xMedia Design System`,
   ...
   **Gap found the same way:** the manifest reported `fonts: []` / `brandFonts: []` — the type pairing
   was only inside `type.html`'s specimen markup, so a project inheriting this system would have got the
   palette with no fonts. `--font-sans` / `--font-display` / `--font-mono` were added to `tokens.css` and
   pushed. **Verification pending:** the manifest is regenerated by the web app's self-check
   (`"source": "spa"`), not by upload, so it stays stale until the project is opened in the web app once.
   See [`docs/design-sync-tool.md`](./docs/design-sync-tool.md) §The self-check runs in the web app.
