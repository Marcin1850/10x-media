...
| Concurrent generation re-read | A **deleted-id `useRef` set** filters every incoming list            | `refreshSummaries` replaces `summaries` wholesale, so ordering alone cannot stop a re-read resurrecting a deleted card — and a `useState` copy captured before an `await` can be older than the deletion itself, so the ref is authoritative. |
