...
## Check the dev-server port before starting one — and again after stopping one

- **Context**: Any local dev-server session (`npm run dev` or equivalent) started **or stopped** during a working session — especially on Windows, where an old process can keep running invisibly across conversation turns, and where stopping the wrapper (`npm`) does not necessarily stop the child it spawned.
- **Problem**: Two shapes, both seen. (1) A dev server from a prior session was already listening on :4321; starting a second `npm run dev` silently fell back to :4322 while the browser kept hitting the stale original, which then didn't pick up a `middleware.ts` edit. Time was spent debugging why an injected fault "wasn't working" before discovering two processes were running. (2) e2e Phase 1, 2026-09-08: stopping the background task reported success, but the `astro dev` child was orphaned and kept holding :4321 — twice in one session. A restart would have silently reused a server carrying the **wrong environment** (`E2E_FAKE_LLM` still set), making the "the seam is off by default" check verify nothing. The same reuse inside the Playwright suite means real OpenRouter calls, because `webServer.env` never reaches a server Playwright did not start.
- **Rule**: Before starting a dev server, check whether one is already listening on the target port (`netstat`/`Get-NetTCPConnection` on Windows, `lsof`/`ss` elsewhere), and reuse it only if its environment matches what you need. After stopping one, check the port **again** and kill the orphan by PID if it is still held — a "successfully stopped" message describes the wrapper, not the listener.
- **Applies to**: implement, impl-review
