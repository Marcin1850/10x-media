...
| S-04 | delete-account            | delete their account and all associated data (GDPR)          | F-01          | Access Control, NFR (privacy) | implementing |
...
- **Status:** implementing (both phases implemented + committed 2026-07-12 — Phase 1 `2ebeaaf`, Phase 2 `2b0c78e`, epilogue `2710a3c`; `SUPABASE_SERVICE_ROLE_KEY` set as a Cloudflare Workers Secret; change `delete-account` → `implemented`, Linear MAR-10 → In Progress). Manual E2E verification + impl-review pending before Done.
...
| S-04       | delete-account            | Delete account + all data (GDPR)               | implementing          | Both phases implemented + committed (2026-07-12); Workers Secret set. Manual E2E + impl-review pending before Done. |
