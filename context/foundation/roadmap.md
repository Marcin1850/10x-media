...
| S-01-fu    | drop-spend-credit-contract | Contract migration: drop legacy `spend_credit()` | blocked               | **Post-S-01 follow-up** (expand/contract). After the new Worker calling `spend_credits` is live on cloud, add a forward migration `drop function if exists public.spend_credit();`. Deferred deliberately — dropping it in S-01's expand migration would open a deploy window. Currently prose-only in S-01 `plan.md` Migration Notes §2 + the `20260719120000` migration's `NOTE (F3)`; this row is its trackable home so it survives S-01 archival. |
