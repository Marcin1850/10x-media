...
| S-08 | persist-video-metadata    | see the video's thumbnail, title, channel, length, upload date and language next to a summary | S-01          | FR-006 (list legibility)      | planned |
...
| S-02       | browse-summary-list       | List of saved summaries                        | no                    | S-01 done; now waiting only on S-08, which is **planned** as of 2026-07-25 (metadata must land first so the list is not retrofitted). S-08 ships **no UI**, so S-02 owns all rendering — including a null-metadata fallback, which it needs regardless since existing rows are not backfilled and a failed metadata fetch produces the same shape. Language is stored as `transcript_lang` (the transcript's language, not the video's) and is deliberately **not** for display. |
