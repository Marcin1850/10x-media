...
| ---------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Catalog invariant** — read `pg_catalog` through the owner connection, compare against a hand-written roster committed in the test file | `src/test/authorization-invariants.int.test.ts` (9 invariants)                                                                                  | The _shape_: every relation in `public` is classified; RLS is on; `anon` holds nothing at all; `authenticated` holds exactly the roster's verbs and no function EXECUTE; every granted verb has one matching policy whose `using` clause is still `auth.uid() = user_id`; `service_role` holds no DML on the internal tables; the schema-wide `anon` default-privilege revoke is intact; `graphql_public` is empty. | That a policy which _exists_ actually filters a live request — and that application code does not reach rows **around** RLS, which the admin client does by design.                                                  |
...
- **Scope every function assertion to `nspname = 'public'`.** `graphql_public.graphql()` _is_ EXECUTE-able by `anon` and `authenticated` by design (`supabase/config.toml:13`); an unscoped query goes red for a reason that is not a finding. The residual worth pinning is that no _object_ appears in `graphql_public` — invariant 9.
...
- **`Dxtm` is signal, and it means different things per role.** `MAINTAIN,REFERENCES,TRIGGER,TRUNCATE` is what the local `postgres` default privileges hand out; it contains no DML. For `authenticated` on a table outside the allow-list its _presence_ is the forgotten-revoke signal and must fail (invariant 4). For `service_role` on the nine internal tables it is inherited and expected, so invariant 8 targets `SELECT/INSERT/UPDATE/DELETE` specifically — a "zero privileges" assertion would go red today for a reason that is not a finding.
