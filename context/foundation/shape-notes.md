...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "decision paralysis — too many options, no basis for filtering without watching"
    - topic: "insight"
      decision: "YouTube recommends too broadly; existing tools summarize single videos but don't help choose which to watch"
    - topic: "auth strategy"
      decision: "login (email+password); multi-tenant architecture but closed registration in MVP"
    - topic: "role model"
      decision: "flat in MVP (single user, full access); architecture prepared for multi-user"
    - topic: "product type"
      decision: "web-app"
    - topic: "target scale"
